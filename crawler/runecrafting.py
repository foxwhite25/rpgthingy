...
url = 'https://wiki.melvoridle.com/index.php/Runecrafting'
headers = {
...
def get_urls(url):
    ...
    black_list = ['/index.php?title=Runecrafting_Skillcape', '/index.php?title=Gunter']
...
def get_data(url):
    ...
    for n in num:
        if a == 0:
            ...
            continue
        if a == 1:
            fr[get_item_id(n)[0]] = int(temp.replace(',', ''))
            a = 0
    ...
    values (?,?,?,?,?,?,?)''', (i, to, fr, time, xp, level, '符文铭刻'))
    conn.commit()
